...
func IsGPURequiredPod(pod *v1.Pod) bool {
	klog.V(9).Infof("Determine if the pod %s needs GPU resource", pod.Name)
	...
	if vcore <= 0 || (vcore < HundredCore && vmemory <= 0) {
		klog.V(9).Infof("Pod %s in namespace %s does not Request for GPU resource",
			pod.Name,
			pod.Namespace)
		return false
	}
...
func IsGPURequiredContainer(c *v1.Container) bool {
	klog.V(9).Infof("Determine if the container %s needs GPU resource", c.Name)
