...
func (alloc *allocator) Allocate(pod *v1.Pod) error {
	newPod := pod.DeepCopy()
...
func (alloc *allocator) Score(pod *v1.Pod) int64 {
	var (
		needCores, needMemory uint
		score, totalScore     int64
	)
	newPod := pod.DeepCopy()
	for _, container := range newPod.Spec.Containers {
		if !util.IsGPURequiredContainer(&container) {
			continue
		}
		needCores = util.GetGPUResourceOfContainer(&container, util.VCoreAnnotation)
		needMemory = util.GetGPUResourceOfContainer(&container, util.VMemoryAnnotation)
		...
		switch {
		case needCores < util.HundredCore:
			score = NewShareMode(alloc.nodeInfo).Score(needCores, needMemory)
			totalScore = totalScore + score
		default:
			score = NewExclusiveMode(alloc.nodeInfo).Score()
			totalScore = totalScore + score
		}
	}
...
// Reserve tries to find a suitable GPU device for containers
// and records some data in pod's annotation
func (alloc *allocator) Reserve(pod *v1.Pod) (*v1.Pod, error) {
	newPod := pod.DeepCopy()
	if newPod.Annotations == nil {
		newPod.Annotations = make(map[string]string)
	}
...
// AllocateOne tries to allocate GPU devices for given container
func (alloc *allocator) ReserveOne(container *v1.Container) ([]*device.DeviceInfo, error) {
	var (
		devs           []*device.DeviceInfo
		sharedMode     bool
		vcore, vmemory uint
